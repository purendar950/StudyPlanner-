...
import {
  ...
  Flame, LayoutDashboard, Menu, Play, Search, Sparkles, Target, TrendingUp, Trophy, X, Zap,
} from 'lucide-react';
...
function App() {
  ...
  const [focus, setFocus] = useState(['Quantitative Aptitude', 'General Awareness']);
...
function PageContent({ active, go, startWizard, completed, setCompleted }: { active: Page; go: (p: Page)=>void; startWizard:()=>void; completed:string[]; setCompleted:React.Dispatch<React.SetStateAction<string[]>> }) {
...
function Dashboard({ go, startWizard, completed, setCompleted }: { go:(p:Page)=>void; startWizard:()=>void; completed:string[]; setCompleted:React.Dispatch<React.SetStateAction<string[]>> }) {
  const done = tasks.filter(t => completed.includes(t.topic) || t.done).length;
  return <div className="content">
    <section className="welcome-row"><div><p className="eyebrow">THURSDAY · AUGUST 28, 2026</p><h1>Good evening, Purendar.</h1><p className="subtle">Your next best study session is ready.</p></div><button className="primary-button" onClick={startWizard}><PlusIcon/> Create study plan</button></section>
    <section className="hero-grid"><div className="focus-card"><div className="card-top"><div><span className="label">TODAY'S FOCUS</span><h2>Time & Work</h2><p>Quantitative Aptitude · 45 min</p></div><div className="focus-icon"><Target size={22}/></div></div><div className="focus-progress"><div><strong>38%</strong><span>mastery</span></div><span>Needs attention</span></div><div className="progress-track"><i style={{width:'38%'}}/></div><button className="light-button" onClick={()=>go('Practice')}>Start focused session <ArrowRight size={17}/></button></div><div className="countdown-card"><div className="countdown-head"><span className="label">SSC CGL 2026</span><span className="status-pill">ON TRACK</span></div><strong className="days">47</strong><span className="days-label">days remaining</span><div className="mini-stats"><div><strong>67%</strong><span>mastery</span></div><div><strong>24h</strong><span>this week</span></div><div><strong>76%</strong><span>accuracy</span></div></div></div></section>
    <div className="section-heading"><div><h2>Today's plan</h2><p>4h 30m planned · {Math.min(done * 45,165)}m completed</p></div><button className="text-button" onClick={()=>go('Plan')}>View full plan <ArrowRight size={16}/></button></div>
    <section className="plan-card"><div className="day-progress"><span>{Math.round((done/6)*100)}%</span><div className="progress-track"><i style={{width:`${Math.round((done/6)*100)}%`}}/></div><span>{done} / 6 tasks</span></div>{tasks.map(task=><div className={`task ${task.done || completed.includes(task.topic) ? 'done':''}`} key={task.time}><span className="task-time">{task.time}</span><button className="task-check" aria-label={`Mark ${task.topic} complete`} onClick={()=>setCompleted(c=>c.includes(task.topic)?c.filter(x=>x!==task.topic):[...c,task.topic])}>{(task.done||completed.includes(task.topic))&&<Check size={14}/>}</button><div className="task-info"><strong>{task.topic}</strong><span>{task.subject}</span></div><span className="task-duration">{task.duration}</span><button className="task-action">{task.done||completed.includes(task.topic)?'Done':'Start'}{!task.done&&!completed.includes(task.topic)&&<ChevronRight size={15}/>}</button></div>)}</section>
    <div className="section-heading compact"><div><h2>Subject mastery</h2><p>Where your next hours will have the most impact.</p></div></div><section className="subject-grid">{subjects.map(s=><article className="subject-card" key={s.name}><div className="subject-title"><div className={`subject-dot ${s.tone}`}/><div><strong>{s.name}</strong><span>Next: {s.next}</span></div><b>{s.value}%</b></div><div className="progress-track"><i className={s.tone} style={{width:`${s.value}%`}}/></div></article>)}</section>
    <section className="bottom-grid"><div className="weekly-card"><div className="section-heading compact"><div><h2>Study consistency</h2><p>Last 7 days</p></div><span className="score">91%</span></div><div className="bars">{['M','T','W','T','F','S','S'].map((d,i)=><div key={`${d}-${i}`}><i style={{height:`${[62,48,82,67,91,36,74][i]}%`}}/><span>{d}</span></div>)}</div></div><div className="revision-card"><div className="revision-icon"><Zap size={20}/></div><div><span className="label">REVISION DUE</span><h3>7 topics need review</h3><p>Protect what you've already learned.</p></div><button className="round-arrow" onClick={()=>go('Revision')}><ArrowRight size={18}/></button></div></section>
  </div>;
}
...
function Wizard({step,setStep,exam,setExam,date,setDate,hours,setHours,level,setLevel,focus,toggleFocus,days,finish,back,search,setSearch,openCategory,setOpenCategory,categories,filtered}:{step:WizardStep;setStep:React.Dispatch<React.SetStateAction<WizardStep>>;exam:typeof exams[0];setExam:React.Dispatch<React.SetStateAction<typeof exams[0]>>;date:string;setDate:React.Dispatch<React.SetStateAction<string>>;hours:number;setHours:React.Dispatch<React.SetStateAction<number>>;level:string;setLevel:React.Dispatch<React.SetStateAction<string>>;focus:string[];toggleFocus:(x:string)=>void;days:number;finish:()=>void;back:()=>void;search:string;setSearch:React.Dispatch<React.SetStateAction<string>>;openCategory:string;setOpenCategory:React.Dispatch<React.SetStateAction<string>>;categories:string[];filtered:typeof exams}) {
